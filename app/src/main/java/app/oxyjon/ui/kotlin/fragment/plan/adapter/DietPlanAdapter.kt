...
class DietPlanAdapter(val mContext: Context) :
    androidx.recyclerview.widget.RecyclerView.Adapter<DietPlanAdapter.ViewHolder>() {
    private val items: ArrayList<PlanListResponse.Data>? = arrayListOf()
    lateinit var onclick: onClickListner

    interface onClickListner {
        fun onSelectDietPlan(id: Int?, planType: String?)
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView!!.setOnClickListener {
            onclick.onSelectDietPlan(items!![position].health_plan_id, items!![position].plan_type)
        }
    }
