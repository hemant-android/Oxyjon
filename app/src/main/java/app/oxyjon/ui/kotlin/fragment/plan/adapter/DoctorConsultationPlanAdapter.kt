...
class DoctorConsultationPlanAdapter(val mContext: Context) :
    androidx.recyclerview.widget.RecyclerView.Adapter<DoctorConsultationPlanAdapter.ViewHolder>() {
    private val items: ArrayList<PlanListResponse.Data>? = arrayListOf()
    lateinit var onclick: onClickListner

    interface onClickListner {
        fun onSelectDoctorConsultationPlan(id: Int?, planType: String?)
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView!!.setOnClickListener {
            onclick.onSelectDoctorConsultationPlan(items!![position].health_plan_id, items!![position].plan_type)
        }
    }
