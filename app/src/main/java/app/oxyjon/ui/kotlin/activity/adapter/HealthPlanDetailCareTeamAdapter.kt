...
class HealthPlanDetailCareTeamAdapter(val mContext: Context) :
    androidx.recyclerview.widget.RecyclerView.Adapter<HealthPlanDetailCareTeamAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        if (items!![position].profile_url != null && !TextUtils.isEmpty(items!![position].profile_url)) {
            ...
            Glide.with(mContext).load(items!![position].profile_url).placeholder(R.drawable.progress_animation)
