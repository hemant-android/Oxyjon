...
class TestReportAdapter(val mContext: Context) :
    androidx.recyclerview.widget.RecyclerView.Adapter<TestReportAdapter.ViewHolder>() {
    private val items: ArrayList<DocumentReportResponse.Data.LabTestReport>? = arrayListOf()
    lateinit var onclick: onClickListner

    interface onClickListner {
        fun onSelectTestReportItem(fileUrl: String?,fileName: String?)
    }
    ...
    fun setData(item: ArrayList<DocumentReportResponse.Data.LabTestReport>) {
        items!!.clear()
        items.addAll(item)
        ...
        if (items!![position].document_url != null && !TextUtils.isEmpty(items!![position].document_url)) {
            ...
            if (items!![position].document_url.contains(".pdf")) {
                ...
                Glide.with(mContext).load(items!![position].document_url)
                    .placeholder(R.drawable.progress_animation).into(holder.imgUpload)
            }
        }
        holder.tvFileName!!.text = items!![position].filename
//        holder.tvType!!.text = "Document Type: " + "Lab test report"
        ...
        holder.itemView.setOnClickListener {
            onclick.onSelectTestReportItem(items!![position].document_url,items!![position].filename)
        }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, position: Int): ViewHolder {
        return ViewHolder(LayoutInflater.from(parent.context)
            .inflate(R.layout.upload_document_layout, parent, false))
    }
